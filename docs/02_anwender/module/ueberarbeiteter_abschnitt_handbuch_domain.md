# Überarbeiteter Abschnitt – docs/module/handbuch_domain.md

Es wurden nur die im Prüfbericht belegten Korrekturen umgesetzt. Unvollständige Entitätstabellen wurden ergänzt, eine zu weit gehende Verallgemeinerung zu `__post_init__` präzisiert, der Parametertyp von `validate_booking_sequence` klargestellt und nicht belegte Festlegungen vorsichtiger formuliert [cite:7][cite:11]. Zusätzlich wurden Aussagen mit fehlender eindeutiger Repository-Evidenz so angepasst, dass sie nur noch durch die geprüften Domain-Dateien gedeckt sind [cite:7][cite:10].

## Neuer, vollständig überarbeiteter Abschnitt
...
ausgedrückt. Entitäten mit fachlichen Invarianten validieren diese in `__post_init__`.
...
| `description`       | `str`                    | Beschreibung der Auffälligkeit                |
...
> Repository-Kommentaren ist für die SQLite-Implementierung eine separate
...
zustandslos auf den übergebenen Listen – kein Datenbankzugriff. `day_bookings` ist
eine chronologisch sortierte `Sequence[BookingType]`, nicht eine Liste vollständiger
`TimeBooking`-Entitäten.
...
- Geändert: „Jede Entität validiert ihre Geschäftsregeln in `__post_init__`“ → „Entitäten mit fachlichen Invarianten validieren diese in `__post_init__`“ (Beleg: `src/arbeitszeit/domain/entities.py`) [cite:7]
- Geändert: `ReviewCase`-Tabelle um `id`, `employee_id`, `description` und `created_at` ergänzt (Beleg: `src/arbeitszeit/domain/entities.py`) [cite:7]
- Geändert: `AuditLogEntry`-Tabelle um `id` und `event_at` ergänzt (Beleg: `src/arbeitszeit/domain/entities.py`) [cite:7]
- Geändert: `WorkScheduleVersion`-Invarianten um `valid_until`-Prüfung ergänzt (Beleg: `src/arbeitszeit/domain/entities.py`) [cite:7]
- Geändert: „SHA-256-Hash der RFID-UID“ → „Hash der RFID-UID“ (Beleg: `src/arbeitszeit/domain/entities.py`) [cite:7]
- Geändert: „durch einen Administrator genehmigt oder abgelehnt werden muss“ → „durch ein Benutzerkonto genehmigt oder abgelehnt werden muss“ (Beleg: `src/arbeitszeit/domain/entities.py`) [cite:7]
- Geändert: Hinweis zu `AuditLogRepository` vorsichtiger formuliert; `autocommit` als dokumentierter Implementierungshinweis statt geprüfte Infrastruktur-Implementierung beschrieben (Beleg: `src/arbeitszeit/domain/ports/repositories.py`) [cite:10]
- Geändert: `validate_booking_sequence` präzisiert: Fehlerarten benannt und Parametertyp von `day_bookings` als `Sequence[BookingType]` klargestellt (Beleg: `src/arbeitszeit/domain/services/booking_rules.py`) [cite:11]
- Markiert als nicht verifizierbar und deshalb vorsichtiger formuliert: konkreter Hash-Algorithmus der RFID-UID (keine eindeutige Repository-Evidenz in der geprüften Domain-Codebasis) [cite:7]
- Markiert als nicht verifizierbar und deshalb vorsichtiger formuliert: Genehmigung eines `Supplement` speziell durch Administratoren (keine eindeutige Repository-Evidenz in der geprüften Domain-Codebasis) [cite:7]
