...
**Offene Lücke:** `SQLiteBackupService` kennt nur `db_path` + `backup_dir`, kein `export_dir`. Die geplante Einbeziehung der Exportdateien (CSV/PDF aus `export.export_dir`, Pflichtenheft v3 §12/§14, Regelwerk v3 §17/§18/§20) ist im implementierten Code noch nicht vorhanden. Schritt 7 gilt erst vollständig abgeschlossen, wenn Exportdateien ebenfalls gesichert werden.
