...
**Schritt 7 Nachtrag (← OFFEN, vor Phase-5-Start):** `SQLiteBackupService` kennt nur `db_path` + `backup_dir`, kein `export_dir`. Die geplante Einbeziehung der Exportdateien (CSV/PDF aus `export.export_dir`, Pflichtenheft v3 §12/§14, Regelwerk v3 §17/§18/§20) fehlt im Code. Umfang: `export_dir: Path | None`-Parameter ergänzen, `create_local_backup()` kopiert Export-Verzeichnis in `backup_dir/exports/`, `scripts/backup.py` erhält `--export-dir`, E2E-Tests erweitern.
