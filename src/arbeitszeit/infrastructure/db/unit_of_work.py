...
class SQLiteUnitOfWork:
    ...
    def __init__(
        ...
        self.device_event_repo: DeviceEventRepository = SQLiteDeviceEventRepository(
            conn
        )
        self.employee_repo: EmployeeRepository = SQLiteEmployeeRepository(conn)
        self.user_account_repo: UserAccountRepository = SQLiteUserAccountRepository(
            conn
        )
        self.rfid_card_repo: RfidCardRepository = SQLiteRfidCardRepository(conn)
        self.time_booking_repo: TimeBookingRepository = SQLiteTimeBookingRepository(
            conn
        )
        self.work_schedule_repo: WorkScheduleRepository = SQLiteWorkScheduleRepository(
            conn
        )
        ...
        self.booking_correction_repo: BookingCorrectionRepository = (
            SQLiteBookingCorrectionRepository(conn)
        )
        self.audit_log_repo: AuditLogRepository = SQLiteAuditLogRepository(
            conn, audit_conn
        )
        self.system_config_repo: SystemConfigRepository = SQLiteSystemConfigRepository(
            conn
        )
