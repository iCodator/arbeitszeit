...
def test_tagesbericht_valides_pdf(conn, export_dir):
    _base_setup(conn)
    path = create_daily_report(conn, date(2025, 6, 1), export_dir, now=_REPORT_NOW)
    assert path.exists()
...
def test_tagesbericht_leerer_tag_erzeugt_pdf(conn, export_dir):
    path = create_daily_report(conn, date(2025, 6, 1), export_dir, now=_REPORT_NOW)
    assert path.exists()
    assert _is_valid_pdf(path)


def test_tagesbericht_verzeichnis_wird_angelegt(conn, export_dir):
...
def test_wochenbericht_valides_pdf(conn, export_dir):
    _base_setup(conn)
    path = create_weekly_report(conn, 2025, 23, export_dir, now=_REPORT_NOW)
    assert path.exists()
    assert _is_valid_pdf(path)
...
def test_monatsbericht_valides_pdf(conn, export_dir):
    _base_setup(conn)
    path = create_monthly_report(conn, 2025, 6, export_dir, now=_REPORT_NOW)
    assert path.exists()
    assert _is_valid_pdf(path)
...
def test_mitarbeiterbericht_filtert_nach_employee(conn, export_dir):
