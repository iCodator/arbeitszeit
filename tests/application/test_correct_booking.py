...
def _cmd(booking_id: int, **overrides: Any) -> CreateCorrectionCommand:
    defaults = dict(
...
def test_buchung_nicht_gefunden_loest_not_found_error() -> None:
    uow = _uow_with_actor()
    uc = CorrectBookingUseCase(_as_uow(uow))
...
def test_buchung_erhaelt_status_corrected() -> None:
    uow, booking_id = _make_uow_with_booking()
    uc = CorrectBookingUseCase(_as_uow(uow))
...
def test_nur_passender_review_case_wird_geschlossen() -> None:
    ...
    case_matching = uow.review_case_repo.add(
        ReviewCase(
            ...
            booking_id=TimeBookingId(booking_id),
            created_at=_EARLIER,
            closed_at=None,
    ...
    case_other = uow.review_case_repo.add(
        ReviewCase(
            ...
            booking_id=TimeBookingId(other_booking.id),
            created_at=_EARLIER,
            closed_at=None,
            closed_by_user_id=None,
        )
...
def test_inaktiver_mitarbeiter_kein_commit_kein_audit_log() -> None:
    uow, booking_id = _make_uow_with_booking()
    emp = uow.employee_repo.get_by_id(1)
    assert emp is not None
    uow.employee_repo._store[1] = dataclasses.replace(emp, is_active=False)
...
def test_manual_entry_review_bleibt_offen_trotz_gleicher_booking_id() -> None:
